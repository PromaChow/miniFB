...
export const addStatus = async (id, name, time, stat, token) => {
  ...
  let res = await axios
    .post("http://127.0.0.1:8080/status", data, {
...
export const addStory = async (id, name, time, form) => {
  ...
  let res = await axios
    .post("http://127.0.0.1:8080/stories", form)
    .then((response) => {
      // alert("yes");
      // console.log("response", response);
      // Cookies.set("token", response.data.access_token);
...
export const getStatus = (token) => {
  const res = axios
    .get("http://127.0.0.1:8080/status", { headers: { Authorization: token } })
    .then((response) => {
      return response;
      //return response;
    })
...
export const getStory = (token) => {
  const res = axios
    .get("http://127.0.0.1:8080/stories", { headers: { Authorization: token } })
    .then((response) => {
      // alert("yes");
      // console.log("response", response);
      // Cookies.set("token", response.data.access_token);
